    ...
    if (
        ...
        for i in range(numberdivisions):
            if num != numberdivisions - 1:
                ...
                df_short = sequences
            newtmpfile = "temp/bigones/UMIintermediate_%s_%s.csv" % (
                barcodenum,
                num + 1,
            )
...
def UMIprocesstheminlots(newdir, barcodenum, numberdivisions):
    ...
    """
    os.chdir(newdir)
    # script_path = '/camp/home/turnerb/home/users/turnerb/code/MAPseq_processing/Preprocessing_sequencing/Bash_scripts/process_big_onesUMI.sh' this one used normally
    script_path = "/camp/home/turnerb/home/users/turnerb/code/MAPseq_processing/Preprocessing_sequencing/Bash_scripts/outofmem_repeat.sh"  # this path used to repeat umi correction for bc36 which runs out of mem so needs hmem nodes
