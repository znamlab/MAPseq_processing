...
def split_samples(
    ...
    fastq_files = unzip_fastq(
        raw_dir, acq_id=acq_id, target_dir=output_dir, overwrite=True
    )
...
if __name__ == "__main__":
    verbose = (
        1  # 0 is silent, 1 prints progress, >2 also prints barcode splitter output
    )
    # Script to take raw data files and split by sample

    camp_root = pathlib.Path(
        "/camp/lab/znamenskiyp/home/shared/projects/turnerb_MAPseq"
    )
